...
for i=1:size(lines,2)-1
    for j=i+1:size(lines,2)
        ...
        if(isApproximatelyEqual(slopei,0,0.2)&&isApproximatelyEqual(slopej,0,0.2)&&isApproximatelyEqual(yi1,yj1,1)&&isApproximatelyEqual(yi2,yj2,1))
            if(xi1>xj1)
                %minx=xj1;
        ...
        elseif(isApproximatelyEqual(slopei,slopej,0.2)&&isApproximatelyEqual(dist1,0,1)&&isApproximatelyEqual(dist2,0,1))%另一条直线上两个点到第一条直线的距离近似0
            if(xi1>xj1)
                %minx=xj1;
                newline=[xj1;xi2;yj1;yi2];
                mergelines=[mergelines,newline];
...
x1=mergelines(1,:); x2=mergelines(2,:);
y1=mergelines(3,:); y2=mergelines(4,:);
dx=x2-x1;
dy=y2-y1;
