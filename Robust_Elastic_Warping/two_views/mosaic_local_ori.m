...
sub_v0_ = max([1, min(box1_2(2,:))]) - margin;
...
z1_ = H(3,1)*x1 + H(3,2)*y1 + H(3,3);
x1_ = (H(1,1)*x1 + H(1,2)*y1 + H(1,3)) ./ z1_;
y1_ = (H(2,1)*x1 + H(2,2)*y1 + H(2,3)) ./ z1_;
gxn = x1_ - x2;
hyn = y1_ - y2;
...
% K(1:n+1:n*n) = lambda * 8*pi * ones(1,n) ./ metrix;
...
%imwrite(mosaic,  [exp_path, '/', 'mosaic_ours.jpg']) ;