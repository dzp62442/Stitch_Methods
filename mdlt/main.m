...
%% 使用指定的寺庙或铁路图片进行拼接，读取预先提取保存的SIFT特征
...
% Images to stitch.
%------------------
path1 = './capture_3/precut_imgs/precut_l.jpg';
path2 = './capture_3/precut_imgs/precut_r.jpg';
...
%---------------------
refine = 1;
if refine==0  % SuperGlue直接得到的匹配
    matches_1 = csvread('./capture_3/superglue_imgs/overlap_0.05_0.4_mkpts0.csv');  
    matches_2 = csvread('./capture_3/superglue_imgs/overlap_0.05_0.4_mkpts1.csv');
else  % 优化后的匹配
    matches_1 = csvread('./capture_3/refine_match/matches_1.csv');  
    matches_2 = csvread('./capture_3/refine_match/matches_2.csv');
end
matches_1 = matches_1';  % 转置
matches_2 = matches_2';
matches_num = length(matches_1);  %有多少组匹配
X = ones(1, matches_num); % 创建一个全为一的数组
data_orig = [matches_1; X; matches_2; X];
...
inliers = 1:1:matches_num;  % 所有点均为内点
